...
import { useGarageManagement, NewGarageInfo } from "@/hooks/useGarageManagement";
...
const Garages = () => {
  ...
  const { fetchGarages, addGarage, fetchLoading, error, garages } = useGarageManagement();
  ...
  const garagesByLocation = filteredGarages.reduce((acc, garage) => {
    ...
    acc[location].push(garage as Garage);
    return acc;
  ...
  const addSampleGarage = async () => {
    try {
      ...
      const sampleGarage: NewGarageInfo = {
